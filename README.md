...
Dev/Demo
--------
If you are running this in a dev environment using `python manage.py runserver`, then you should probably make sure `DEBUG=True` is set in settings.py. In DEBUG mode, staticfiles handles the serving of css, js, etc. Otherwise if `DEBUG=False`, you will need to set MEDIA\_ROOT, STATIC\_ROOT, then run `python manage.py build_static` (this can overwrite existing files in STATIC_DIR). Note that the application will be noticeably slower when DEBUG is turned on.
